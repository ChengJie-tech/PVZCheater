...
DWORD monitorThreadFunc(LPVOID lpThreadParameter) {
	while (1) {
		...
		if (g_dlg->m_bnSun.GetCheck()) { // 需要无限阳光
			DWORD value = 9990;
			WriteMemory(&value, sizeof(value), 0x00731C50, 0x868, 0x5578, -1);
		}
...
void CPVZCheaterDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_KILL, m_bnKill); // 绑定
...
// 绑定
afx_msg void CPVZCheaterDlg::OnBnClickedCourse() {
	ShellExecute(NULL, CString("open"), CString("https://www.baidu.com/"), NULL, NULL, SW_SHOWNORMAL);
}

afx_msg void CPVZCheaterDlg::OnBnClickedKill() {
	...
	if (m_bnKill.GetCheck()) {
		...
		WriteMemory(data, sizeof(data), 0x00545DFA);
	} else {
		// 不需要
		BYTE data[] = { 0x28, 0x6C, 0X24, 0X20 };
		WriteMemory(data, sizeof(data), 0x00545DFA);
	}
}

afx_msg void CPVZCheaterDlg::OnBnClickedSun() {
	/*if (m_bnSun.GetCheck()) {
		log("勾选了");
	}
	else {
